...
function AlertDialogContent({
  ...
  // Keep translate(-50%,-50%) in every transform keyframe so fixed centering survives motion overrides.
  const tx = 'translate(-50%, -50%)';

  // Avoid asChild on Radix Content: Slottable + Slot merge with motion.div triggers Children.only.
  return (
    ...
    >
      <motion.div
        ...
        initial={{
          opacity: 0,
          filter: 'blur(4px)',
          transform: `${tx} perspective(500px) ${rotateAxis}(${initialRotation}) scale(0.8)`,
        }}
        animate={{
          opacity: 1,
          filter: 'blur(0px)',
          transform: `${tx} perspective(500px) ${rotateAxis}(0deg) scale(1)`,
        }}
        exit={{
          opacity: 0,
          filter: 'blur(4px)',
